...
class Qwen3VLVisionModel(Qwen3VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor,  visual_tensor_to_save:dict[str,torch.Tensor] = None, **kwargs) -> torch.Tensor:
        """
        Args:
        ...
        """
        if visual_tensor_to_save is not None:
            visual_tensor_to_save["initial_hidden_states"] = hidden_states.clone().contiguous()
            
        hidden_states = self.patch_embed(hidden_states)
        if visual_tensor_to_save is not None:
            visual_tensor_to_save["after_patch_embed"] = hidden_states.clone().contiguous()

        if visual_tensor_to_save is not None:
            visual_tensor_to_save["grid_thw"] = grid_thw.clone().contiguous()
                
        pos_embeds = self.fast_pos_embed_interpolate(grid_thw)
        if visual_tensor_to_save is not None:
            visual_tensor_to_save["pos_embeds"] = pos_embeds.clone().contiguous()

        hidden_states = hidden_states + pos_embeds  
        if visual_tensor_to_save is not None:
            visual_tensor_to_save["after_hidden_states_plus_pos_embeds"] = hidden_states.clone().contiguous()

        rotary_pos_emb = self.rot_pos_emb(grid_thw)
        if visual_tensor_to_save is not None:
            visual_tensor_to_save["rotary_pos_emb"] = rotary_pos_emb.clone().contiguous()
        ...
        cu_seqlens: torch.Tensor = F.pad(cu_seqlens, (1, 0), value=0)
